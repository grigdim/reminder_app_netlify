  ...
  return (
    <li className="reminder">
      <div className="reminderInfo">
        <label
          style={reminder.checked ? { textDecoration: "line-through" } : null}
        >
          <h4>{`Reminder Title: ${reminder.title}`}</h4>
          <p className="date">{`This event is on: ${format(
            new Date(reminder.date),
            "MMMM dd, yyyy pp"
          )}`}</p>
          {reminder.frequency !== "do not repeat" && (
            <p>
              This reminder has a {reminder.frequency} frequency and has been
              triggered {reminder.triggered}{" "}
              {reminder.triggered === 1 ? "time" : "times"}
            </p>
          )}
        </label>
      </div>
      <div className="buttons">
        <button
          ...
          style={
            reminder.checked
              ? { backgroundColor: "rgba(0, 89, 255, 0.75)" }
              : { backgroundColor: "rgba(0, 200, 0, 0.75)" }
          }
        >
          {reminder.checked ? "Uncheck" : "Check"}
        </button>
